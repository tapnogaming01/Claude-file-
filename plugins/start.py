...
async def start_cmd(client: Client, message: Message):
    ...
    if len(args) < 2:
        return await message.reply_text(
            "Hi! Tap a batch button in the channel and I'll send you those "
            "episodes here in DM."
        )
    ...
    for ep_no in range(start_ep, end_ep + 1):
        ...
        if file_id in sent_file_ids:
            continue  # combined files cover multiple episodes, don't resend the same file
        sent_file_ids.add(file_id)
        try:
            await client.send_cached_media(
                chat_id=message.chat.id,
                file_id=file_id,
                caption=f"{story_name} \u2014 Episode {ep_no}",
            )
            sent_count += 1
        except Exception:
            pass
